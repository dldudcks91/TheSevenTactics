...
const PartyTab = {
    ...
    _render() {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            html += `<div class="party-slot ${filled ? 'filled' : ''}"
                data-action="pick-slot" data-slot="${i + 1}">
                <div class="party-slot-label">슬롯 ${i + 1}</div>
                ${filled ? `<div class="party-slot-name">${slot.hero_name || slot.hero_id || '?'}</div>
                    <div class="party-slot-meta">${slot.grade || ''} · ${slot.faction || ''}</div>` :
                    `<div class="party-slot-name" style="color:var(--text-muted)">비어있음</div>`}
            </div>`;
        }
        html += '</div>';
